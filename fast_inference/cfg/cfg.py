...
def add_rl_args(p: ArgumentParser):
    ...
    # debugging options
    p.add_argument("--benchmark", default=False, type=ast.literal_eval, help="Benchmark mode")
...
def add_model_args(p: ArgumentParser):
    ...
    p.add_argument("--use_rnn", default=True, type=ast.literal_eval, help="Whether to use RNN core in a policy or not")
    p.add_argument(
        "--rnn_size",
        default=512,
        type=int,
...
def add_default_env_args(p: ArgumentParser):
    ...
    p.add_argument("--use_env_info_cache", default=False, type=ast.literal_eval, help="Whether to use cached env info")
    p.add_argument(
        "--env_gpu_actions",
        default=False,
        type=ast.literal_eval,
...
def add_wandb_args(p: ArgumentParser):
    ...
    p.add_argument("--with_wandb", default=False, type=ast.literal_eval, help="Enables Weights and Biases integration")
    p.add_argument(
        "--wandb_user",
        default=None,
        type=str,
...
def add_pbt_args(p: ArgumentParser):
    ...
    p.add_argument("--with_pbt", default=False, type=ast.literal_eval, help="Enables population-based training (PBT)")
    p.add_argument(
